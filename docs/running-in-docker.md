...
**Timeline previews are the media server's, not prdb-fab's.** The strip of
thumbnails a player shows while you drag along the seek bar is generated by
Jellyfin out of the Video File itself, and it is switched on per library, in
Jellyfin's own settings, as trickplay image extraction. prdb-fab deliberately
writes nothing of the kind beside a file: a preview placed there replaces the
server's own rather than adding one, and the server's is made from the original
at the interval it tells its clients about. `docs/media-server-timeline-previews.md`
records the run that established this. The user previews prdb publishes are
shown inside prdb-fab instead — in a Video's Preview and in the Review queue.
...
| --- | --- |
| `0.22.0` | A release. This is what documentation and Compose files should pin. |
